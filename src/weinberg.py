...
for ensemble_dir, ens_key in ensemble_map.items():
    ...
    try:
        ...
        with open(os.path.join(ensemble_dir, "meson_extraction_f_ps_samples.json")) as f:
            f_ps_data = json.load(f)
        with open(os.path.join(ensemble_dir, "meson_extraction_f_av_samples.json")) as f:
        ...
        with open(os.path.join(ensemble_dir, "meson_extraction_as_ps_samples.json")) as f:
            as_ps_data = json.load(f)
        with open(os.path.join(ensemble_dir, "meson_extraction_as_av_samples.json")) as f:
            as_av_data = json.load(f)
        with open(os.path.join(ensemble_dir, "meson_extraction_as_v_samples.json")) as f:
    ...
    results.append({
        "representation": "f", "Ens": ens_key,
        "s_0": obs_f["s_0"][0], "s_0_err": obs_f["s_0"][1],
        "s_1": obs_f["s_1"][0], "s_1_err": obs_f["s_1"][1],
        "s_2": obs_f["s_2"][0], "s_2_err": obs_f["s_2"][1],
    })
    ...
    results.append({
        "representation": "as", "Ens": ens_key,
        "s_0": obs_as["s_0"][0], "s_0_err": obs_as["s_0"][1],
        "s_1": obs_as["s_1"][0], "s_1_err": obs_as["s_1"][1],
        "s_2": obs_as["s_2"][0], "s_2_err": obs_as["s_2"][1],
    })
...
for rep in ["f", "as"]:
    for ens in ["M1", "M2", "M3", "M4", "M5"]:
        ...
        print(f"$({{{rep}}})$-type     & {ens} & {fmt(r['s_0'], r['s_0_err'])} & {fmt(r['s_1'], r['s_1_err'])} & {fmt(r['s_2'], r['s_2_err'])} \\\\")
    if rep == "f":
        print("        \\hline")
