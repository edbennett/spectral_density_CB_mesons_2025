...
use rule output_template as CSVs_to_tables_meson_GEVP with:
    ...
    output:
        latex=expand(
            "assets/tables/{ensemble}_output_table_aE{n}_meson.tex",
            ensemble=ensembles,
            n=[0, 1, 2],
        ),
...
use rule output_template as CSVs_to_tables_CB_GEVP with:
    ...
    output:
        latex=expand(
            "assets/tables/{ensemble}_output_table_CB_aE{n}.tex",
            ensemble=ensembles,
            n=[0, 1, 2],
        ),
...
use rule output_template as CSVs_to_tables_meson_matrixelements with:
    ...
    output:
        latex=expand(
            "assets/tables/{ensemble}_output_table_matrix_mesons.tex",
            ensemble=ensembles,
        )
...
use rule output_template as CSVs_to_tables_CB_matrixelements with:
    ...
    output:
        latex=expand(
            "assets/tables/{ensemble}_output_table_matrix_CB.tex",
            ensemble=ensembles,
        ),
...
use rule output_template as renormalise with:
    ...
    output:
        mesons_tex=expand(
            "assets/tables/renormalised_{ensemble}_output_table_matrix_mesons.tex",
            ensemble=ensembles,
        ),
        cb_tex=expand(
            "assets/tables/renormalised_{ensemble}_output_table_matrix_CB.tex",
            ensemble=ensembles,
        ),
